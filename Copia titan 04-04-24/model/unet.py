...
class Unet(nn.Module):
  def __init__(self, in_channels, n_feat=512, max_text_len=512, height=64, cond_drop_prob=0.1, device=None):  # cfeat - context features
    ...
    self.down = nn.ModuleList([#UnetDown(n_feat//16, n_feat//8, time_cond_dim=time_cond_dim, text_cond_dim=n_feat, device=device),    # down1 (b, 64, 32, 32)
                 UnetDown(n_feat//8, n_feat//4, time_cond_dim=time_cond_dim, text_cond_dim=n_feat, self_attn=True ,device=device),                   # down2 (b, 128, 16, 16)
                 UnetDown(n_feat//4, n_feat//2, time_cond_dim=time_cond_dim, text_cond_dim=n_feat, self_attn=True, device=device),                   # down3 (b, 256, 8, 8)
                 UnetDown(n_feat//2, n_feat, time_cond_dim=time_cond_dim, text_cond_dim=n_feat, self_attn=True, device=device)])                     # down4 (b, 512, 4, 4)
    ...
    self.up = nn.ModuleList([UnetUp(2*n_feat, n_feat//2, time_cond_dim=time_cond_dim, text_cond_dim=n_feat, self_attn=True, device=device),       # up1 (b, 256, 8, 8)
               UnetUp(n_feat, n_feat//4, time_cond_dim=time_cond_dim, text_cond_dim=n_feat, self_attn=True, device=device),                         # up2 (b, 128, 16, 16)
               UnetUp(n_feat//2, n_feat//8, time_cond_dim=time_cond_dim, text_cond_dim=n_feat,self_attn=True, device=device),                      # up3 (b, 64, 32, 32)

...
class Diffusion_Models():
  ...
  def __init__(self, timesteps, beta1=1e-4, beta2=0.02, height=128):
  ...
  def unorm(self, img):
    mean_ = 0.5140147805213928
    std_ = 0.3199092149734497
    img = (img * std_) + mean_
  ...
  @torch.no_grad()
  def sample_ddpm_context(self, model, n_sample, lowres_img, save_rate=20, end_timestep = None):
    # x_T
    ...
    lowres_img = [resize(x, (self.height, self.height)) for x in lowres_img]
    lowres_img = torch.cat(lowres_img).view(n_sample, 3, self.height, self.height).to(self.device)

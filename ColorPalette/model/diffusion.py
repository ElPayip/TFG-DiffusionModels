...
class Diffusion_Models():
  ...
  @torch.no_grad()
  def sample_ddpm_context(self, model, n_sample, orig_img, save_rate=20, end_timestep = None):
    # x_T
    ...
    orig_img = torch.cat(orig_img).view(n_sample, 3, self.height, self.height).to(self.device)
    ...
    for i in range(end_timestep, 0, -1):
        ...
        eps = model(samples, t, orig_img)    # predict noise e_(x_t,t, ctx)

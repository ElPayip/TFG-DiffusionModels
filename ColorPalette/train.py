...
def saturacion_batch(img, sat, df):
    return adjust_saturation(df.unorm(img.permute(0,2,3,1)).permute(0,3,1,2), sat)
...
def train(model, diffusion, optimizer, train_dataset, validation_dataset=None, batch_size=32, epochs=100, device=DEVICE):
    ...
    log_file=METRICS_DIR+'training_progress_color.txt'
    ...
    for epoch in range(epochs):
        ...
        if epoch%5==0 or epoch == int(epochs-1):
            torch.save(model.state_dict(), SAVE_DIR + "model_color_{:03d}.pth".format(epoch + 1))
            show_msg('saved model at ' + SAVE_DIR + "model_color_{:03d}.pth".format(epoch + 1))
            
        metrics = dict()
        metrics['train'] = list_total_loss
        metrics['validation'] = list_total_loss_val
        np.save(f'{METRICS_DIR}color.npy', metrics)